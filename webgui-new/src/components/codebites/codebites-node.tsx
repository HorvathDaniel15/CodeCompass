...
export const CodeBitesNode = ({ data }: NodeProps<DataProps>): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      const initFileInfo = await getFileInfo(
        data.astNodeInfo.range?.file as string,
      );
      createClient(appCtx.workspaceId, initFileInfo?.type);
      const initText = await getSourceText(data.astNodeInfo.id as string);
      sendGAEvent({
  ...
  const handleClick = async () => {
    ...
    const astNodeStartLine = data.astNodeInfo?.range?.range?.startpos
      ?.line as number;
    ...
    const newAstNodeInfo = await getAstNodeInfoByPosition(
    ...
    const newAstNodeRefTypes = await getReferenceTypes(
    ...
    const nodeAlreadyPresent = nodes
      .map((node) => node.id)
      .includes(newAstNodeDef.id as string);
  ...
  const handleDelete = () => {
    const nodes = reactFlowInstance.getNodes();
    const edges = reactFlowInstance.getEdges();
    const currentNode = reactFlowInstance.getNode(
      data.astNodeInfo.id as string,
    ) as Node;

    const relatedNodes = getOutgoers(currentNode, nodes, edges) as Node[];
    const relatedNodeIds = relatedNodes.map((node) => node.id);

    const relatedEdges = getConnectedEdges(
      [...relatedNodes, currentNode],
      edges,
    );
    ...
    const updatedNodes = nodes.filter(
      (node) =>
        !relatedNodeIds.includes(node.id) && node.id !== currentNode?.id,
    );
    const updatedEdges = edges.filter(
      (edge) => !relatedEdgeIds.includes(edge.id),
    );
  ...
  const customOffsetGutter = gutter({
    ...
    lineMarker(view, line, others) {
      ...
      const number =
        ...
        (data.astNodeInfo.range?.range?.startpos?.line ?? 0) -
        1;
  ...
  return (
    <SC.NodeOuterContainer>
      ...
      <SC.NodeHeader>
        <Tooltip
          title={fileInfo ? (fileInfo.path as string) : "Unknown file path"}
          placement={"top"}
        >
          <SC.NodeTitle>
            {fileInfo ? (fileInfo.name as string) : "Unknown file"}
          </SC.NodeTitle>
        </Tooltip>
      ...
      <ReactCodeMirror
        readOnly={true}
        extensions={
          [customOffsetGutter, languageExtension(fileInfo?.type)].filter(
            Boolean,
          ) as Extension[]
        }
...
const languageExtension = (fileType?: string): Extension | null => {
  switch (fileType) {
    case "CPP":
      return cpp();
    case "CS":
