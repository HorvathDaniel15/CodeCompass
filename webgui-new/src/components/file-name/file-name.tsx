...
export const FileName = ({
  ...
  const [buildLogAnchorEl, setBuildLogAnchorEl] = useState<HTMLElement | null>(
    null,
  );
  ...
  const renderFileReferences = async () => {
    createClient(appCtx.workspaceId, info?.type);
    const fileRefTypes = await getFileReferenceTypes(info?.id as string);

    const elements: React.ReactNode[] = [];
    for (const [key, value] of fileRefTypes) {
      const refCountForType = await getFileReferenceCount(
        info?.id as string,
        value,
      );
      const refsForType = await getFileReferences(info?.id as string, value);
      ...
      {info ? (
        <>
          ...
          >
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {buildLog.map((log, idx) => (
                          <SC.BuildLogTableRow
                            key={idx}
                            onClick={() => jumpToBuildLog(log.range as Range)}
                          >
                            <TableCell>
                              {buildLogMessages[log.messageType ?? "0"]}
                            </TableCell>
            ...
            <div>{filePath}</div>
            {gitBlameEnabled && (
              <Button onClick={() => appCtx.setGitBlameInfo([])}>
                {t("fileName.hideGitBlame")}
              </Button>
            )}
          </SC.StyledDiv>
